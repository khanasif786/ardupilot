...
void AP_Camera_Backend::init()
{
    ...
    if (_params.track_enable == 1) {
        gcs().send_text(MAV_SEVERITY_WARNING, "CAMERA TRACKING BACKEND INITIALISED");
        printf("Initialised");
        tracker = NEW_NOTHROW AP_Camera_Tracking();
    } else {
        gcs().send_text(MAV_SEVERITY_CRITICAL, "CAMERA TRACKING BACKEND INITIALISATION FAILED");
